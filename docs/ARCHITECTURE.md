...
3. **Coverage mask (occlusion-aware):** a removed-candidate voxel only counts if session B actually *saw* that space: voxel center inside a B keyframe frustum, within range, **and** the camera→voxel segment passes no occupied voxel of B's own grid (half-voxel-step march, target's 1.5-voxel neighborhood exempt). Frustum-only checking ghosts occlusion shadows — the floor hidden under a newly added object flags as "removed" and can even pair with the true addition into a fake "shifted" (found empirically; see RESUME.md entry 5). Symmetric for added vs A's grid. Unobserved space is a coverage gap, reported separately, never a change.
4. **Regions:** 26-connected components over flagged voxels, drop components < `minRegionVoxels` (4) or mean support confidence < `minConfidence` (0.45 — calibrated: grazing-angle sampling flicker ghosts measured ≤ 0.37 confidence, genuine changes ≥ 0.51, across both scenario families). Per region: bbox, centroid, `volumeM3 = voxelCount·s³`, confidence = mean point-support saturation.
